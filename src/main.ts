...
const storage = ((process.env.MONGO_URI) ? new MongoDbStorage(
...
controller.ready(() => {
  ...
  if (controller.plugins.cms) {
    controller.on('message,direct_message', async (bot, message) => controller.plugins.cms.testTrigger(bot, message));
  }
});
